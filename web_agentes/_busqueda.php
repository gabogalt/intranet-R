...
<head>
  ...
  <title>RE/MAX Rio - Búsqueda</title>
  <meta charset="UTF-8">
      ...
      <div class="row">
        <div class="col ">
            ...
            <h2>Resultados de la búsqueda</h2>
              ...
              $str = strtolower($_GET['TEXTO']);
              $strings = explode(' ',$str);
              
              foreach($archivos as $archivo){
                if(substr($archivo,0,1) != '_' and strpos($archivo,".php") and $archivo != 'index1.php' and $archivo != 'index2.php' and $archivo != 'index1_.php'){
                  $contents = file_get_contents($archivo);
                  
                  if($_GET['TIPO'] == 'EXACTO'){
                  
                    if(strpos(strtolower($contents),$str)){
                      $ini = strpos($contents,"<title>RE/MAX Rio - ")+20;
                      $fin = strpos($contents,"</title>");
                      echo "<li><a href='$archivo'>". substr($contents,$ini,$fin - $ini)."</a></li>";
              //        echo $archivo. "<br>";
                    }
                  }else{
                    $ok = true;
                    foreach($strings as $string){
                      if(!strpos(strtolower($contents),$string) and $string != ''){
                        $ok = false;
                      }
                    }
                    if($ok){
